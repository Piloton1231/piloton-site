...
- 複数のYouTubeプレイヤー方式を順番に試し、実際に応答する `googlevideo.com` URLだけへ転送
- メイン経路からKsyncへは自動転送しない（KsyncはWeb画面上の明示的な予備リンクとして提供）
...
- Vercelなど共有クラウドのIPはYouTubeに拒否される場合があります。その場合、メイン経路はエラーを返すため、Web画面からKsync予備リンクを選択します。
